---
title: "Data splitting & Data resampling"
author: "Andrew Kim"
date: "6/22/2022"
output: html_document
---

__This notebook contains the data splitting and resampling procedures for the tree cover type data set.__
...
```{r}
# Resampling
# Generate info needed for three resampled versions of the training set
set.seed(100)
CV_repeatSplit <- createDataPartition(ctraining, p = 0.80, times = 3)
str(CV_repeatSplit)
```

```{r}
set.seed(100)
Cov_Splits <- createFolds(ctraining, k = 10, returnTrain = TRUE)
Cov_Splits
```

```{r}
Cov_firstrow <- Cov_Splits[[1]]
```

```{r}
Cov_cv <- ctraining[Cov_firstrow]
nrow(ctraining)
```
